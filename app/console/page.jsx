  ...
  return (
    <>
      <div className="card code-card">
        <div className="card-title">우리 업체코드</div>
        <div className="card-desc">손님이 <b>착한거래 동의하기</b>에서 이 코드를 입력하면 우리 회원사 앞으로 동의가 등록됩니다.</div>
        <div className="code-row">
          <span className="code-val">{code || "—"}</span>
          {code && <button className="btn btn-sm" onClick={() => copyText(code)}><Icon name="file" /> 코드 복사</button>}
          <button className="btn btn-sm" onClick={() => copyText(`${location.origin}/consent`)}>동의 링크 복사</button>
        </div>
      </div>
            ...
            list.map((c) => (
              <div className="risk-row" key={c.id}>
                <div>
                  ...
                  <div className="meta">{fmtDate(c.createdAt)} · {c.status === "completed" ? (c.self ? "손님 직접 동의" : "동의완료") : "대기"}</div>
                </div>
                <div className="sp" />
                {c.status === "completed"
      ...
      <div className="card">
        ...
        {showTpl && (
          <>
            <div className="card-desc"><b>계약서는 그대로 두고</b>, 기존 「개인정보 수집·이용 동의서」에 아래 <b>제3자 제공 동의 항목만 별도로</b> 추가하면 됩니다. (디지털 동의 링크를 못 쓰는 경우 대안) <b>이 방식으로 받은 동의서는 회원사가 직접 보관·관리</b>하며, 착한거래 플랫폼에는 기록되지 않습니다.</div>
