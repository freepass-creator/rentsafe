...
const BUILTIN = {
  "dudguq@gmail.com": { company: "착한거래 관리자", role: "admin", code: "" },  // 관리자(나만)
...
export async function signup({ company, email, pw, bizFile, service = "" }) {
  const e = (email || "").trim().toLowerCase();
  ...
  if (FB_READY) {
    try {
      ...
      await setDoc(doc(db, "members", cred.user.uid), {
        company: company.trim(), email: e, role: "member", code, service: service.trim(), bizFile, createdAt: serverTimestamp(),
      });
  ...
  const code = genCode();
  acc[e] = { pw, company: company.trim(), role: "member", code, service: service.trim(), bizFile, createdAt: Date.now() };
  saveAccounts(acc);
