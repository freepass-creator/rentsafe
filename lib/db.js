...
import { db, FB_READY } from "./firebase";
import {
...
// FB_READY 단일 출처(firebase.js): app 초기화 성공 && !DEMO_MODE. 불일치/널-데레프 방지.
export { FB_READY };
...
const DEMO_MEMBERS = [
  { code: "100001", company: "스피드렌터카", service: "렌터카" },
  { code: "100002", company: "테스트렌터카", service: "렌터카" },
  { code: "100003", company: "하나모빌리티", service: "렌터카" },
];

// 거래코드로 회원 찾기 → { company, code, service } 또는 null(=미등록)
export async function findMemberByCode(code) {
  const c = (code || "").trim();
  if (!c) return null;
  // 데모 회원은 모드(Firebase/localStorage) 무관하게 항상 인식 — 시연이 Firestore 없이도 동작
  const demo = DEMO_MEMBERS.find((m) => m.code === c);
  if (demo) return { company: demo.company, code: c, service: demo.service || "" };
  if (IS_LOCAL) {
    ...
    return fromAcc ? { company: fromAcc.company, code: c, service: fromAcc.service || "" } : null;
  }
  const ms = await getDocs(query(collection(db, "members"), where("code", "==", c)));
  if (ms.empty) return null;
  const d = ms.docs[0].data();
  return { company: d.company, code: c, service: d.service || "" };
}
...
export async function createSelfConsent({ name, phone = "", company, code = "", verified, signed = false, cert = null, photos = null }) {
  if (IS_LOCAL) {
    const arr = lsGet(LS_C);
    const id = rid("cs_");
    arr.unshift({ id, name, phone, company, code, status: "completed", verified, signed, cert, photos, self: true, createdAt: Date.now(), completedAt: Date.now() });
    lsSet(LS_C, arr);
    return id;
  }
  const ref = await addDoc(collection(db, "consents"), {
    name, phone, company, code, status: "completed", verified, signed, cert, photos, self: true,
