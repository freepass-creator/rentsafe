...
export async function createSelfConsent({ name, phone = "", company, code = "", verified, signed = false, cert = null }) {
  if (IS_LOCAL) {
    const arr = lsGet(LS_C);
    const id = rid("cs_");
    arr.unshift({ id, name, phone, company, code, status: "completed", verified, signed, cert, self: true, createdAt: Date.now(), completedAt: Date.now() });
    lsSet(LS_C, arr);
    return id;
  }
  const ref = await addDoc(collection(db, "consents"), {
    name, phone, company, code, status: "completed", verified, signed, cert, self: true,
