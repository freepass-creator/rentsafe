...
// DEMO_MODE 동안은 localStorage(오프라인) — 실서비스 시 DEMO_MODE=false → Firestore
export const FB_READY = !!process.env.NEXT_PUBLIC_FB_PROJECT_ID && !DEMO_MODE;
...
export async function createSelfConsent({ name, phone = "", company, code = "", verified, signed = false }) {
  if (IS_LOCAL) {
    const arr = lsGet(LS_C);
    const id = rid("cs_");
    arr.unshift({ id, name, phone, company, code, status: "completed", verified, signed, self: true, createdAt: Date.now(), completedAt: Date.now() });
    lsSet(LS_C, arr);
    return id;
  }
  const ref = await addDoc(collection(db, "consents"), {
    name, phone, company, code, status: "completed", verified, signed, self: true,
