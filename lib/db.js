...
const DEMO_MEMBERS = [
  { code: "SPEED1", company: "스피드렌터카" },
  { code: "TEST01", company: "테스트렌터카" },
  { code: "HANA01", company: "하나모빌리티" },
];

// 업체코드로 회원사 찾기 → { company, code } 또는 null(=비회원사)
export async function findMemberByCode(code) {
  ...
  if (IS_LOCAL) {
    ...
    const fromAcc = Object.values(acc).find((a) => (a.code || "").toUpperCase() === c);
    return DEMO_MEMBERS.find((m) => m.code === c) || (fromAcc ? { company: fromAcc.company, code: c } : null);
  }
  const snap = await getDocs(query(collection(db, "members"), where("code", "==", c)));
  if (snap.empty) return null;
  const d = snap.docs[0].data();
  return { company: d.company, code: c };
}
