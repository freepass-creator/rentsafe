...
export async function findMemberByCode(code) {
  ...
  if (IS_LOCAL) {
    ...
    const perm = DEMO_MEMBERS.find((m) => m.code === c) || (fromAcc ? { company: fromAcc.company, service: fromAcc.service || "" } : null);
    return perm ? { company: perm.company, code: c, service: perm.service || "" } : null;
  }
